...
import { useAddGovernanceBodyMutation, useUpdateGovernanceBodyMutation } from 'src/api'


const AddGovernanceBodyModal = ({ close, visible, details }) => {

  const [values, setValues] = useState({});
  const [errors, setError] = useState({})
  const [errorText, setErrorText] = useState({})
  const [createMutation, {isLoading, error, isError, isSuccess}] = useAddGovernanceBodyMutation()
  ...
  useEffect(() => {
    ...
    if(isError || updateIsError){{
      setErrorText({
        ...errorText,
        response:(error.data?.data && error.data?.data[0]?.uiMessage) ||  (updateError.data?.data && updateError.data?.data[0]?.uiMessage)
      })
    }}
  ...
  const handleSubmit = (event) => {
    ...
    if(details){
      updateMutation({governanceBodies: [{...values}]})
    }else{
      createMutation({governanceBodies:payload})
    }
  }
...
export default AddGovernanceBodyModal;
